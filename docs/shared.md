...
需要创建一个新的端点/进程以允许终端用户在登陆后会被重定向到投注选择（和已填充的投注单）。

This will require a change to the Login API. 这需要对登录 API 进行修改。

Implementation as suggested below: 请按照以下建议实施:
...
After receving selection info from B2B Website, the selection will be added into the Bet Slip, the error responses will be the same as the error responses from Bet Slip when placing bets. 在B2B网站接收到选择信息后，该选择将被添加到投注单中，错误响应将与下注时投注单的错误响应相同。

| Error Code 错误代码 <br/> | Description 说明 <br/> |
| --- | --- |
| `ALL_BETTING_CLOSED` | Betting is not allowed at this moment 当前不允许投注 |
| `B000` | Bet placement  service error 投注服务错误 |
| `BETTING_BUDGET_EXCEEDED` | You have exceeded your betting budget! 您的投注额已超出额度！ |
| `CORRELATED` | This selection is correlated with another selection  这个选项与另外一个选项相关 |
| `ERROR1004` | An error occurred 发生错误 |
| `ERROR1005` | An error occurred 发生错误 |
| `ERROR3105` | Bet was not accepted because the event is currently offline 投注未被接受，因为赛事目前处于离线状态 |
| `EVENT_OFFLINE` | This selection is currently offline  该选项目前处于离线状态 |
| `INSUFFICIENT_FUNDS` | You do not have sufficient funds to place these bets  您没有足够的资金进行这些投注 |
| `INVALID_BET_ACCEPTANCE_TYPE` | The line is no longer available for betting 该比赛不再提供投注 |
| `INVALID_LEGS` | One or more legs are invalid 一个或多个选项无效 |
| `INVALID_SELECTION` | One or more selections are invalid 一个或多个选项无效 |
| `LINE_CHANGED` | The odds changed.  Please resubmit and confirm your bet. 赔率已更改。请重新提交并确认您的投注。 |
| `LINE_ODDS_CHANGED` | The odds changed.  Please resubmit and confirm your bet. 赔率已更改。请重新提交并确认您的投注。 |
| `MAX_PICKS` | You cannot add more than 10 selections to your Bet Slip. 您不能在投注单上添加超过 10 项选择。|
| `NOT_ALL_AVAILABLE` | Not all of your selections are available for Multiple bets. 您选择的选项并非所有都可用于复式投注。 |
| `RESUBMIT_REQUEST` | Bet not accepted. Please try again or remove this selection from your Bet Slip. 投注不被接受。请重试或从投注单中删除此选项。 |
| `SUSPENDED_LINE` | Your account has been suspended. Please contact Customer Service for help. 您的账户已被暂停。请联系客户服务寻求帮助。|
| `SUSPENDED_LINE_CREDIT` | Your account has been suspended. Please contact your Upline for help. 您的账户已被暂停。请联系您的上线寻求帮助。 |
| `SYSTEM_ERROR_3` | Unexpected error or System error 意料之外的错误或系统错误 |
| `ROUND_ROBIN_DISALLOWED` | Round robin is not allowed in one of the leagues. 其中一个联赛不被允许在轮询过关投注中使用。 |
| `UNAVAILABLE` | The odds are no longer available for betting. 赔率已不再可供投注。 |
| `UNSUPPORTED_MULTIPLES` | This selection is not available for Multiple bets. 此选项不适用于复式投注。|
